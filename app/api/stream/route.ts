...
GUI DESIGN REGELN:
- Dunkler Hintergrund: Color3.fromRGB(15, 15, 20)
- Abgerundete Ecken: UICorner mit CornerRadius = UDim.new(0, 12)
- UIStroke für Ränder
- TweenService für Animationen
- UIGradient für Hintergründe
- Font.GothamBold für Titel, Font.Gotham für Text
- Immer einen Close Button mit X
- Moderne cleane Designs
...
Für Diamanten: Neon, hellblau, leicht transparent
...
export async function POST(req: NextRequest) {
  ...
  if (plan === "pro") {
    ...
    if (used >= PRO_LIMIT) {
      return new Response("Monatslimit erreicht! Upgrade auf Ultimate für mehr Anfragen.", { status: 429 });
    }
  } else if (plan === "unlimited") {
    const month = new Date().toISOString().substring(0, 7);
    key = `credits:${userId}:${month}`;
    const used = (await redis.get<number>(key)) || 0;
    if (used >= ULTIMATE_LIMIT) {
    ...
    const used = (await redis.get<number>(key)) || 0;
    if (used >= FREE_LIMIT) {
      return new Response("Tageslimit erreicht! Upgrade auf Pro für mehr Anfragen.", { status: 429 });
    }
  }
    ...
    async start(controller) {
      ...
      if (match) {
        try {
          ...
          if (key) {
            const used = (await redis.get<number>(key)) || 0;
            await redis.set(key, used + 1, { ex: plan === "pro" || plan === "unlimited" ? 2592000 : 86400 });
          }
